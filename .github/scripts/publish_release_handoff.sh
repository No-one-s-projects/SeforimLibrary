...
if len({p.name for p in paths})!=len(paths): raise SystemExit('duplicate local asset names')
remote=v.get('assets',[]); actual={a['name']:(a['size'],a.get('digest')) for a in remote}
if len(actual)!=len(remote) or set(actual)-{p.name for p in paths}: raise SystemExit('unexpected remote assets')
for p in paths:
 h=hashlib.sha256(p.read_bytes()).hexdigest(); expected=(p.stat().st_size,'sha256:'+h)
